...
class Member(object):
    ...
    def dominates(self, other):
        ...
        """
        comp1 = [x <= y for x, y in zip(self.objective_values, other.objective_values)] + [self.psd <= other.psd,
                                                                                           self.dsd <= other.dsd]
        comp2 = [x < y for x, y in zip(self.objective_values, other.objective_values)] + [self.psd < other.psd,
                                                                                          self.dsd < other.dsd]
    ...
    def is_in_sectors(self, sector):
        ...
        for start, end in sector:
            if end <= 2 * np.pi:
                in_this_sector = all(
                    [start <= x < end for x in self.polar_objective_values[1:]]
                )
            else:
                in_this_sector = all(
                    [
                        (start <= x < 2 * np.pi or 0 <= x < end - 2 * np.pi)
                        for x in self.polar_objective_values[1:]
                    ]
                )

...
def run():
    ...
    nsgaiii = NSGA3(moop, 700, 250, 0.9, 2, 20, 20, 4)
    ...
    # run the algorithm
    nsgaiii.run()
    front = np.array(nsgaiii.fast_non_dominated_sort(nsgaiii.population)[0])
    front = np.array([member.objective_values for member in front])
    distance_mean, distance_std = nsgaiii.evaluate_distance_metric()
    print("Distance metric mean: ", distance_mean)
    print("Distance metric std: ", distance_std)
    diversity = nsgaiii.evaluate_diversity_metric()
    ...
    ax.set_zlim(0, 1)
    plt.legend()
    plt.savefig(f"images/{problem_name}_nsga3.png")
    plt.close()

    # create a GIF from the gif_images in the gif_images directory
    create_gif("gif_images", f"./gifs/{problem_name}_nsga3.gif")

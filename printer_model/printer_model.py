import data.exploration_params as exploration_params
...
def predict_printer_colors(ink_area_coverages: npt.NDArray,
    ...
    predicted_spectras = compute_neugebauer(colorant_area_coverages, ng_primary_reflectances)

    # Convert spectra to XYZ
    predicted_xyz_colors = color_processing._spectrum_2_xyz(predicted_spectras, d65_illuminant, xbar, ybar, zbar)
    ...
    for i in range(predicted_lab_colors.shape[0]):
        predicted_rgb_colors[i, :] = color.lab2rgb([predicted_lab_colors[i, 0],
...
def compute_neugebauer(colorant_area_coverage: npt.NDArray,
                       ng_primary_reflectances: npt.NDArray) -> npt.NDArray:
...
if __name__ == "__main__":
    ...
    [test_rgb_colors,
     test_xyz_colors,
     test_lab_colors] = predict_printer_colors(points_ds_test,
    ...
    visualization.save_lab_gamut(test_lab_colors, os.path.join('plots', exploration_params.name), 'gamut_full', 'Full printer gamut')
    ...
    points_ds_p0 = genetic_algorithm.get_random_parameters(exploration_params.limits_ds,
    ...
    [_,
     xyz_colors_p0,
     points_ps_p0] = predict_printer_colors(points_ds_p0,
                                               exploration_params.ng_primary_reflectances,
                                               exploration_params.white_ciexyz,
                                               exploration_params.d65_illuminant,
                                               exploration_params.xbar,
    ...
    #
    visualization.save_lab_gamut(points_ps_p0, plot_dir, 'gamut_a_initial', 'Initial gamut (area=%.3f)' % p0_area)
    ...
    for i in range(exploration_params.iterations):
        ...
        obj2 = _diversity_mult_obj(points_ds_pi, exploration_params.k_n, exploration_params.limits_ds)
        obj3 = _diversity_mult_obj(points_ps_pi[:, 1:3], exploration_params.k_n, exploration_params.limits_ps[1:3])
        obj_scores_pi = np.column_stack((obj1, obj2, obj3))

        #
        _, scores_pi, _, _ = sorting.sort_nd(points_ps_pi,
        ...
        #
        points_ds_qi = genetic_algorithm.generate_offsprings(points_ds_pi,
                                                             exploration_params.pop_size,
        ...
        [_,
         xyz_colors_qi,
         points_ps_qi] = predict_printer_colors(points_ds_qi,
                                                exploration_params.ng_primary_reflectances,
                                                exploration_params.white_ciexyz,
                                                exploration_params.d65_illuminant,
                                                exploration_params.xbar,
        ...
        print("Gamut area of all samples (after iteration=%i) is %.6f" % (i, current_area))

        visualization.save_lab_gamut(points_ps_all,
                                     plot_dir,
                                     'gamut_after_i%i' % i,
        ...
        obj2 = _diversity_mult_obj(points_ds_ri, exploration_params.k_n, exploration_params.limits_ds)
        obj3 = _diversity_mult_obj(points_ps_ri[:, 1:3], exploration_params.k_n, exploration_params.limits_ps[1:3])
        obj_scores_ri = np.column_stack((obj1, obj2, obj3))

        #
        _, _, sorted_ids_ri, _ = sorting.sort_nd(points_ps_ri,
                                                 exploration_params.start_angle_range,
                                                 exploration_params.cone_count_range,
                                                 obj_scores_ri)
    ...
    plt.plot(np.arange(exploration_params.iterations + 1), np.repeat(test_area, exploration_params.iterations + 1), label="Base Line")
